---
title: "How to Train Your Classifier (`crc-version`)"
author: "Dan Villarreal, Lynn Clark, Jennifer Hay, and Kevin Watson"
date: "Original 24 July 2019; Last modified 4 October 2021"
output: 
...
params:
  ...
  SLURM_job-id: NULL
  SLURM_nodeid: NULL
  SBATCH_partition: NULL
  SBATCH_nodes: NULL
  SBATCH_tasks-per-node: NULL
  SBATCH_cpus-per-task: NULL
  SBATCH_mem: NULL
  testing: FALSE
...
This document was adapted from the [`main` branch version](https://github.com/nzilbb/How-to-Train-Your-Classifier/blob/main/How_to_Train_Your_Classifier.Rmd) to test the performance of the [Center for Research Computing](https://crc.pitt.edu/) high-performance computing cluster, and to save the data for later. Because Pitt's HPC cluster setup differs from the University of Canterbury HPC cluster on which this document was originally run, the output data in this document isn't identical to that of the [original](https://nzilbb.github.io/How-to-Train-Your-Classifier/How_to_Train_Your_Classifier.html). Information on cluster performance (i.e., timing) can be found [below](#Generate_test-script_summary). This repo also contains the [Slurm shell script](https://github.com/nzilbb/How-to-Train-Your-Classifier/blob/crc-version/smp-shell-script.sh) that was used to run this document.


# Introduction

This guide walks readers through the process of training a random-forest classifier for automated coding of sociophonetic variation, using the statistical computing language R [@RCoreTeam2018] and the packages `ranger` [@WrightZiegler2017] and `caret` [@Kuhn2018]. This guide is intended to be a companion to our manuscript "From categories to gradience: Auto-coding sociophonetic variation with random forests", and it recreates the classifier of Southland English non-prevocalic /r/ discussed in the manuscript. By consulting [the R Markdown file that generated this document](https://github.com/nzilbb/How-to-Train-Your-Classifier/blob/crc-version/How_to_Train_Your_Classifier.Rmd), readers can run the code on their own systems to recreate the /r/ classifier. The classifier file can also be found [here](https://github.com/nzilbb/How-to-Train-Your-Classifier/blob/main/LabPhonClassifier.Rds).
...
```{r imputation, class.output="displayCode", cache=params$testing}
...
```{r markOutliers, class.output="displayCode", cache=params$testing}
trainingData <-
  trainingData %>%
  group_by(Speaker, Vowel) %>% 
...
```{r fstInit, class.output="displayCode", cache=params$testing}
...
```{r fstListResamples, class.output="displayCode", cache=params$testing}
...
```{r fstListMethods, class.output="displayCode", cache=params$testing}
...
```{r fstListFoldsReps, class.output="displayCode", cache=params$testing}
...
```{r fstListTuneGrid, class.output="displayCode", cache=params$testing}
...
```{r smryTuneGrid, class.output="displayCode", cache=params$testing}
...
```{r fstListNumTrees, class.output="displayCode", cache=params$testing}
...
```{r fstTuned, class.output="displayCode", cache=params$testing}
registerDoSEQ()
set.seed(302302)
...
```{r fstNoOutlier, class.output="displayCode", cache=params$testing}
registerDoSEQ()
set.seed(302302)
...
```{r fstListDropOut0, class.output="displayCode", cache=params$testing}
cl <- makeCluster(5)
registerDoParallel(cl)
clusterSetRNGStream(cl, 302302)
...
```{r fstListDropOut1_1, class.output="displayCode", cache=params$testing}
...
```{r fstListDropOut2_1, class.output="displayCode", cache=params$testing}
...
```{r fstListDropOut2_2, class.output="displayCode", cache=params$testing}
...
```{r fstListDropOut3_1, class.output="displayCode", cache=params$testing}
...
```{r fstListDropOut4_1, class.output="displayCode", cache=params$testing}
...
```{r fstListDropOut5_1, class.output="displayCode", cache=params$testing}
...
```{r fstListDropOut6_1, class.output="displayCode", cache=params$testing}
...
```{r fstListDropOut7_1, class.output="displayCode", cache=params$testing}
...
```{r fstListDropOut7_2, class.output="displayCode", cache=params$testing}
...
```{r fstListDropOut7_3, class.output="displayCode", cache=params$testing}
...
```{r finalClassifier, class.output="displayCode", cache=params$testing}
registerDoSEQ()
set.seed(302302)
